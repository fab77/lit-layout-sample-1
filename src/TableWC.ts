...
export class TableWC extends LitElement {
  ...
  handleProxyEvent(event: ProxyEvent): void {
    ...
    if (event.eventType === 'unlinked' && event.targetId) {
      this.links.delete(event.targetId);
      this.links = new Set(this.links);
      this.sentDatasets = this.sentDatasets.filter(dataset => dataset.targetId !== event.targetId);
      if (this.sendTargetId === event.targetId) {
        this.sendTargetId = '';
      }
      this.rebuildShownIntoWidgetByRow();
      this.requestUpdate();
  ...
  removeRowFromDataset(targetId: string, datasetId: string, rowIndex: number): void {
    ...
    this.rebuildShownIntoWidgetByRow();
    this.propagateDatasetUpdate(datasetId, previousTargets);
    if (this.sendTargetId === targetId && this.datasetIdInput.trim() === datasetId) {
      const selectedDataset = this.sentDatasets.find(
        dataset => dataset.targetId === this.sendTargetId && dataset.datasetId === datasetId,
      );
      this.checkedRows = this.data.map((_, index) => selectedDataset?.rowIndexes.includes(index) ?? false);
    }
  }

  propagateDatasetUpdate(datasetId: string, previousTargets: string[] = []): void {
    if (!this.widgetId) return;
    const datasetsToSync = this.sentDatasets.filter(dataset => dataset.datasetId === datasetId);
    const currentTargets = Array.from(new Set(datasetsToSync.map(dataset => dataset.targetId)));
    const targetIds = Array.from(new Set([...currentTargets, ...previousTargets]));

    for (const targetId of targetIds) {
      const targetDataset = datasetsToSync.find(dataset => dataset.targetId === targetId);
      const rows = targetDataset ? targetDataset.rowIndexes.map(index => this.data[index]) : [];
      const payload: RowsPayload = targetDataset
        ? {
            ...
            rows,
            rowIndexes: [...targetDataset.rowIndexes],
          }
        : {
            sourceId: this.widgetId,
            datasetId,
      ...
      widgetsProxy.emit(targetId, 'rows-selected', payload);
    }
  }
  ...
  sendSelectedRows(): void {
    ...
    if (existingTargets.length > 0) {
      const nextSentDatasets = this.sentDatasets.map(dataset => {
        if (dataset.datasetId !== datasetId) return dataset;
        return { ...dataset, rowIndexes: selectedRowIndexes };
      });
      const targetIdsToUpdate = [...existingTargets];
      const canAddSelectedTarget = !!selectedTargetId && this.links.has(selectedTargetId);

      if (canAddSelectedTarget && !targetIdsToUpdate.includes(selectedTargetId)) {
        nextSentDatasets.push({ targetId: selectedTargetId, datasetId, rowIndexes: selectedRowIndexes });
        targetIdsToUpdate.push(selectedTargetId);
      }

      this.sentDatasets = nextSentDatasets;

      for (const targetId of targetIdsToUpdate) {
        widgetsProxy.emit(targetId, 'rows-selected', payload);
      }

      if (canAddSelectedTarget) {
        this.sendTargetId = selectedTargetId;
      }
    } else {
  ...
  deleteDatasetEverywhere(): void {
    ...
    this.sentDatasets = this.sentDatasets.filter(dataset => dataset.datasetId !== datasetId);
    this.rebuildShownIntoWidgetByRow();
    this.checkedRows = this.data.map(() => false);
    if (this.datasetIdInput.trim() === datasetId) {
      this.datasetIdInput = '';
    }
  }
  ...
  render() {
    ...
    const datasetAlreadyExists = this.sentDatasets.some(dataset => dataset.datasetId === this.datasetIdInput.trim());
    const canSend = hasSelectedRows && hasDatasetId && (datasetAlreadyExists || !!this.sendTargetId);
    const canDeleteEverywhere = hasDatasetId && datasetAlreadyExists;
    ...
    return html`
      ...
      <div>
        ...
        />
        <button @click=${this.sendSelectedRows} ?disabled=${!canSend}>Send/Update dataset</button>
        <button @click=${this.removeDataset} ?disabled=${!this.sendTargetId || !hasDatasetId}>Remove dataset</button>
        <button @click=${this.deleteDatasetEverywhere} ?disabled=${!canDeleteEverywhere}>Delete dataset</button>
      </div>
      <p>
